...
def _dataframe_to_json_safe_dict(df):
    """Convert a pandas DataFrame to a JSON-serializable dictionary by converting Timestamp keys to strings"""
    ...
    try:
        if isinstance(df_copy.index, pd.DatetimeIndex):
            # Convert DatetimeIndex to string index
            df_copy.index = pd.Index([ts.strftime('%Y-%m-%d') if isinstance(ts, pd.Timestamp) else str(ts) for ts in df_copy.index])
        elif len(df_copy.index) > 0:
            # Check if any index value is a Timestamp
            if any(isinstance(x, pd.Timestamp) for x in df_copy.index):
                df_copy.index = pd.Index([str(ts) if isinstance(ts, pd.Timestamp) else ts for ts in df_copy.index])
    except (AttributeError, IndexError, TypeError):
        # Fallback: convert all index values to strings
        try:
            df_copy.index = pd.Index([str(x) for x in df_copy.index])
        except:
            pass
    
    # Convert column names to string if they contain Timestamps
    try:
        if len(df_copy.columns) > 0 and any(isinstance(x, pd.Timestamp) for x in df_copy.columns):
            df_copy.columns = [str(ts) if isinstance(ts, pd.Timestamp) else ts for ts in df_copy.columns]
    ...
    # Recursively convert any remaining Timestamp values to strings
    def convert_timestamps(obj):
        if isinstance(obj, pd.Timestamp):
            return obj.isoformat()
        elif isinstance(obj, dict):
            return {str(k) if isinstance(k, pd.Timestamp) else k: convert_timestamps(v) for k, v in obj.items()}
    ...
    while retries < max_retries:
        try:
            ...
            try:
                ...
                # Use helper function to convert DataFrames to JSON-serializable dicts
                fundamentals = {
                    'balance_sheet': _dataframe_to_json_safe_dict(balance_sheet),
                    'income_statement': _dataframe_to_json_safe_dict(income_stmt),
                    'cash_flow': _dataframe_to_json_safe_dict(cash_flow)
                }
                ...
                try:
                    ...
                    if recommendations is not None and not recommendations.empty:
                        fundamentals['recommendations'] = _dataframe_to_json_safe_dict(recommendations)
